...
const insertIntoDB = async (
  data: StudentEnrollCourse
): Promise<StudentEnrollCourse> => {
  // Use Prisma to find the first record in the 'studentEnrolledCourse' table that matches certain conditions.
  const isCourseOngoingOrCompleted = await prisma.studentEnrollCourse.findFirst(
    {
      where: {
        OR: [
          // Check if the 'status' property of the record is equal to 'ONGOING'.
          {
            status: StudentEnrollCourseStatus.ONGOING,
          },
  ...
  const result = await prisma.studentEnrollCourse.create({
    data,
    include: {
...
const getAllFromDB = async (
...
): Promise<IGenericResponse<StudentEnrollCourse[]>> => {
  ...
  const whereConditions: Prisma.StudentEnrollCourseWhereInput =
  ...
  const total = await prisma.studentEnrollCourse.count({
    where: whereConditions,
  });
...
const getByIdFromDB = async (
  id: string
): Promise<StudentEnrollCourse | null> => {
  const result = await prisma.studentEnrollCourse.findUnique({
    where: {
      id,
    },
    include: {
...
const updateOneInDB = async (
  id: string,
  payload: Partial<StudentEnrollCourse>
): Promise<StudentEnrollCourse> => {
  const result = await prisma.studentEnrollCourse.update({
    where: {
      id,
    },
    data: payload,
...
const deleteByIdFromDB = async (id: string): Promise<StudentEnrollCourse> => {
  const result = await prisma.studentEnrollCourse.delete({
    where: {
      id,
    },
    include: {
